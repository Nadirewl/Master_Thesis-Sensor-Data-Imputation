...
import optuna #optuna should be installed or added to poetry first)
...
# Initialize Mlflow
mlflow.set_experiment("VAE Train2")
...
def train(epoch,lambda_val):
    ...
    for batch_idx, data in (pbar := tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0)):
        ...
        target_column[:] = -2  #mask the target column
        recon_column = recon_batch[:, targetcol_idx].cpu().detach().numpy()
        mse_loss_target_train += mean_squared_error(target_column, recon_column)
...
#Adding Optuna,and SHAP
#Optuna 


# Define run name
vae_run_name = "vae_for_pr_cyl_max__0"
...
# 1. Prepare shap_df
shap_df = numeric_df.copy()

# 2. Prepare background
background = shap_df.copy()
background = background.apply(pd.to_numeric, errors='coerce').dropna()

# 3. Prepare sample
sample_to_explain = shap_df.apply(pd.to_numeric, errors='coerce').dropna()

# 4. Turn into tensors
background_tensor = torch.tensor(background.values, dtype=torch.float32).to(device)
sample_tensor = torch.tensor(sample_to_explain.values, dtype=torch.float32).to(device)

# 5. Model wrapper
model.eval()
class TargetColumnModel(torch.nn.Module):
    def __init__(self, model, targetcol_idx):
        super(TargetColumnModel, self).__init__()
        self.model = model
        self.targetcol_idx = targetcol_idx

    def forward(self, x):
        recon_batch, _, _ = self.model(x)
        return recon_batch[:, self.targetcol_idx].unsqueeze(1)

target_model = TargetColumnModel(model, targetcol_idx)

# 6. Check model outputs to avoid NaNs/Infs
with torch.no_grad():
    # Background
    bg_output = target_model(background_tensor)
    bg_nan_mask = (torch.isnan(bg_output) | torch.isinf(bg_output)).any(dim=1).cpu().numpy()
    background = background[~bg_nan_mask]

    # Sample
    sample_output = target_model(sample_tensor)
    sample_nan_mask = (torch.isnan(sample_output) | torch.isinf(sample_output)).any(dim=1).cpu().numpy()
    sample_to_explain = sample_to_explain[~sample_nan_mask]

# 7. Recreate tensors after filtering
if len(background) == 0 or len(sample_to_explain) == 0:
    raise ValueError("Background or sample has no valid rows after cleaning!")

background_tensor = torch.tensor(background.values, dtype=torch.float32).to(device)
sample_tensor = torch.tensor(sample_to_explain.values, dtype=torch.float32).to(device)

print(f"Background size after cleaning: {background_tensor.shape}")
print(f"Sample size after cleaning: {sample_tensor.shape}")

# 8. DeepExplainer
explainer = shap.DeepExplainer(target_model, background_tensor)
...
shap.summary_plot(shap_values[0].cpu().numpy(), sample_to_explain, feature_names=shap_df.columns)