...
#This is for the presentation in the end
df=pd.read_parquet("/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/df_for_presentation.parquet")
...
#Note: df1 is data without deleting time column, if time index is going to be used later for comparison 
...
mlflow.set_experiment("GP_VAE_Final")
...
# Run Optuna study
mlflow.set_experiment("GP-VAE-Presentation")
with mlflow.start_run(run_name="GPVAE_presentation)") as parent_run:
...
saving_path="/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/gp_vae/model_presentation"
if os.path.exists(saving_path):
    shutil.rmtree(saving_path)
    os.makedirs(saving_path) 
...
with mlflow.start_run(run_name="GPVAEforpresentation", nested=True):
    GPVAE_best= GPVAE(
        n_steps=data_reshaped.shape[1],
        n_features=data_reshaped.shape[2],
        latent_size=37, #should be the latent dimensions 
    ...
    torch.save(GPVAE_best.model.state_dict(), "GPVAE_weights2.pth")
    ...
    print(metrics_df)
    metrics_df.to_parquet("/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/GPVAE_metrics2.parquet", index=False)
...
# === CONFIGURATION ===
original_data_path = "/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/df_for_presentation.parquet"
model_weights_path = "/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/GPVAE_weights2.pth"
output_path = "/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor_imputation_thesis/nadire/imputed_wizGPVAE2.parquet"
n_steps = 20
...
data_reshaped = data.reshape(-1, n_steps, n_features)

# === MODEL SETUP ===
device = 'cuda' if torch.cuda.is_available() else 'cpu'

# Define best_params manually or load from config
best_params = {
    "beta": 0.6568903882476151,
    "epochs":34,
    "length_scale": 4.881546647934297,
    "batch_size": 128,
    "lr": 0.0005404144686687628
}
