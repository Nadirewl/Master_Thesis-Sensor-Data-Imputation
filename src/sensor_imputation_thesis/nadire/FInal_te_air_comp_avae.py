...
#Drop columns with different indexes 
df = df.drop(
    columns=[
        "te_air_comp_in_a__1",
        "te_air_comp_in_a__2",
...
def main():
   ...
   # Specify the target column, here should be changed while training other target columns
    # (edit point as needed)
    targetcolumn = "te_air_comp_in_a__0"
    ...
    ##Debugging part 
    print("Scaling parameters (min):",data_min)
    print("Scaling parameters(scale):",scale)
    ...
    vae_run_name = "optuna_vae_adjusted_for_te_air_comp_in_a__0"
    ...
    with mlflow.start_run(run_name="Final_test_val_for_te_air_comp_in_a__0", nested=True):
            # Model Reloading
