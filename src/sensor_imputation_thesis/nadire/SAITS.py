...
# Artificially mask. Mask 20% of the data (MIT part), try 30% to compare with GP-VAE
...
mlflow.set_experiment("SAITS_Final(rf)_Tocompare")
...
time_col = df['time'].values[:len(data)]
imputed_df = pd.DataFrame(imputed_flat, columns=df.columns.drop('time'))
imputed_df['time'] = time_col
imputed_df = imputed_df[['time'] + list(imputed_df.columns.drop('time'))]  # Reorder

# === Save the final imputed dataset ===
imputed_df.to_parquet("/home/ec2-user/SageMaker/sensor-imputation-thesis/src/sensor-imputation-thesis/nadire/dffinal_for_comparison.parquet", index=False)
...
#Impute original data with missing values with the best saits model 

def impute_dataframe_with_saits(
    df: pd.DataFrame,
    ...
    """
    Impute a DataFrame using a trained SAITS model (no metrics, just imputation).

    Args:
        df (pd.DataFrame): Input DataFrame with NaNs.
        model (torch.nn.Module): Trained SAITS model.
        feature_names (list): Columns to include in the imputation.
        scaler (optional): Scaler used during model training.
        step (int): Step size for optional downsampling (e.g., 10 = every 10th row).
        seq_len (int): Sequence length used by SAITS model.

    Returns:
        pd.DataFrame: Imputed DataFrame (downsized if needed, no metrics).
    """
    ...
    # Reshape into 3D tensor: [batch, seq_len, features]
    total_len, n_features = data_norm.shape
    n_batches = total_len // seq_len
    usable_len = n_batches * seq_len

    data_norm = data_norm[:usable_len]
    data_norm_3d = data_norm.reshape(n_batches, seq_len, n_features)

    # Replace NaNs with zero (SAITS handles missing via attention mask)
    x_input = np.nan_to_num(data_norm_3d, nan=0.0)

    # Run through SAITS
    model.eval()
    with torch.no_grad():
        x_tensor = torch.tensor(x_input).to(next(model.parameters()).device)
        imputed_tensor = model(x_tensor)
        imputed_array = imputed_tensor.cpu().numpy()

    # Inverse transform if needed
    if scaler:
        imputed_flat = imputed_array.reshape(-1, n_features)
        imputed_flat = scaler.inverse_transform(imputed_flat)
    else:
        imputed_flat = imputed_array.reshape(-1, n_features)

    # Return imputed DataFrame with original feature names
    imputed_df = pd.DataFrame(imputed_flat, columns=feature_names)
    imputed_df.index = df.iloc[:usable_len].index  # preserve index after cut

    return imputed_df


#Reload best model
model=saits_best.to(device)
model.eval()

feature_names=df1[sensor_cols].columns.tolist()
impute_dataframe_with_saits(   
    df: pd.DataFrame,
    model,
